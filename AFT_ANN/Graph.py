...
class Graph:
    ...
    def AFt_meshing(self, net):
        self.sort_front()
        # 当阵面不空
        while self.frontnum > 0:
            # 取出阵面生成单元
            print('还剩' + str(self.frontnum) + '个')
            selected_node = self.generate_tri(self.front_list[0], net)
            if selected_node != None:
                ...
                self.al = self.al * 1.2
                continue
        plt.savefig('./result.jpg', dpi=3600, format='eps')

    def new_point_ann(self, face: Face, net):
        neighbors1 = face.node1.neighbors
        neighbors2 = face.node2.neighbors
        neighbors2.remove(face.node1)
        neighbors1.remove(face.node2)
        temp_x = [face.node1.x, face.node2.x]
        temp_y = [face.node1.y, face.node2.y]
        for i in neighbors1:
            temp_x.append(i.x)
            temp_y.append(i.y)
        for i in neighbors2:
            temp_x.append(i.x)
            temp_y.append(i.y)
        temp_x = deleteDuplicatedElementFromList3(temp_x)
        temp_y = deleteDuplicatedElementFromList3(temp_y)

        # 先缩放后旋转再平移
        dist = face.distance
        ab = [(face.node2.x - face.node1.x) / dist,
              (face.node2.y - face.node1.y) / dist]
        self.rotate_angle = math.acos(
            ab[0]) if (ab[1] < 0) else -math.acos(ab[0])
        self.scale = 1. / dist
        # 平移
        self.x_offset = temp_x[0]
        self.y_offset = temp_y[0]
        for j in range(0, len(temp_x)):
            temp_x[j] = temp_x[j] - self.x_offset
            temp_y[j] = temp_y[j] - self.y_offset
        ...
        for j in range(0, len(temp_x)):
            ...
            y_pre = temp_y[j]
            temp_x[j] = x_pre * \
                math.cos(self.rotate_angle) - y_pre * \
                math.sin(self.rotate_angle)
            temp_y[j] = y_pre * \
                math.cos(self.rotate_angle) + x_pre * \
                math.sin(self.rotate_angle)

        # 缩放
        for j in range(0, len(temp_x)):
            temp_x[j] = temp_x[j] * self.scale
            temp_y[j] = temp_y[j] * self.scale

        # 平移
        x_offset = temp_x[0] - 1.0
        y_offset = temp_y[0] - 1.0
        for j in range(0, len(temp_x)):
        ...
        # zero padding
        if len(temp_x) < 10:
            for m in range(0, 10 - len(temp_x)):
                temp_x.append(0.0)
                temp_y.append(0.0)
        coords = []
        for i in range(0, 10):
            coords.append(temp_x[i])
            coords.append(temp_y[i])
        x_out, y_out = net(torch.Tensor(coords))
        x_out = x_out.item()
        y_out = y_out.item()
        # 平移
        x_out += 1.0
        ...
        # 旋转
        x = x_out * \
            math.cos(self.rotate_angle) + y_out * math.sin(self.rotate_angle)
        y = y_out * \
            math.cos(self.rotate_angle) - x_out * math.sin(self.rotate_angle)
        # 缩放
        x /= self.scale
        y /= self.scale
        # 平移
        x += self.x_offset
        y += self.y_offset
        print('face ' + str(face.node1.x) + ' ' + str(face.node1.y) + ' ' +
              str(face.node2.x) + ' ' + str(face.node2.y))
        print('x: ' + str(x) + ' y: ' + str(y))
        new_node = Node(id=-1, x=x, y=y)
        return new_node, face.distance * 0.866
    ...
    def generate_tri(self, face: Face, net):
        # new_node, sp = self.new_point_tri(face)
        new_node, sp = self.new_point_ann(face, net)
